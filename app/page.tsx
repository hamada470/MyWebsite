...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-gray-800 pb-12" style={{ direction: 'rtl', textAlign: 'right' }}>
      ...
      {/* زر الواتساب العائم - في اليمين */}
      <a 
        href="https://wa.me/966569962482" 
        target="_blank" 
        rel="noopener noreferrer"
        className="fixed bottom-10 right-4 z-50 bg-green-600 hover:bg-green-700 text-white p-3 rounded-full shadow-2xl flex items-center justify-center"
        aria-label="Whatsapp"
      >
        <MessageCircle className="w-7 h-7" />
      </a>

      {/* زر الاتصال العائم - في اليسار */}
      <a 
        href="tel:0569962482" 
        className="fixed bottom-10 left-4 z-50 bg-green-600 hover:bg-green-700 text-white p-3 rounded-full shadow-2xl flex items-center justify-center"
        aria-label="Call"
      >
        <Phone className="w-7 h-7" />
      </a>

      {/* الشريط السفلي نحيف وشريط اتصال مصغر */}
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-gray-900 text-white py-1 px-2 text-center shadow-lg border-t border-gray-800">
        <a 
          href="tel:0569962482" 
          className="text-xs font-semibold text-yellow-400 hover:underline inline-flex items-center justify-center gap-2"
        >
